...
//		console.log("cnt="+cnt);
		for(var i=0;i<cnt;i++){
			...
			function showHP(){
				...
				cxt.beginPath();
				cxt.fillStyle = "red";
				cxt.arc(438,144,15,0,2*Math.PI,true);
				cxt.fill();
				cxt.fillStyle = "white";
				cxt.fillText(computerHP,434,148);
				cxt.restore();
				cxt.closePath();
				cxt.save();
				cxt.beginPath();
				cxt.fillStyle = "red";
				cxt.arc(440,500,15,0,2*Math.PI,true);
				cxt.fill();
				cxt.fillStyle = "white";
				
				cxt.fillText(heroHP,435,504);
				cxt.restore();
				cxt.closePath();
...
//						console.log("turnover="+turnover);
			...
			function drawCrystal(){
				...
				cxt.beginPath();
				cxt.fillStyle = "blue";
				cxt.rect(523,20,45,30);
//				cxt.fill();
				cxt.fillStyle = "white";
				cxt.fillText(+computerNowCrystal+"/"+computerMaxCrystal,540,42);
...
//					cxt.beginPath();
//					
//					cxt.arc(600+i*18,555,8,0,2*Math.PI,true);
//					cxt.fillStyle = "#000";
//					cxt.stroke();
//					
//					cxt.closePath();
...
////				
				//画您的最大个法力水晶 灰底
				for(var i=0;i<yourMaxCrystal;i++){	
					cxt.beginPath();
				...
				for(var i=0;i<yourNowCrystal;i++){	//画您拥有的法力水晶 蓝色
					cxt.beginPath();
				
					cxt.arc(600+i*18,555,8,0,2*Math.PI,true);
					cxt.fillStyle = "#6666CC";
					cxt.fill();
					cxt.closePath();
				...
				cxt.save();
				cxt.fillStyle = "blue";
				cxt.rect(540,545,50,20);
//				cxt.fill();
				cxt.fillStyle = "white";
				cxt.fillText(yourNowCrystal+"/"+yourMaxCrystal,555,560);
				cxt.restore();
				cxt.closePath();
				cxt.restore();
			}
...
//				cxt.rect(beginx,beginy,cardWidth,cardHeight);
				...
				if(obj.posi==2){
					cxt.fillStyle= "red";
					cxt.arc(obj.posx+70,obj.posy+110,10,0,Math.PI*2,true);
					cxt.fill();
					cxt.fillStyle = "white";
					cxt.fillText(obj.HP,obj.posx+65,obj.posy+115);
					if(obj.posi==2&&obj.sleeping==1){
						cxt.fillStyle = "deepskyblue";
						var basegp = 40;
						var gp = basegp+obj.attack+obj.HP;
						if(allT%gp>=gp/4){
							cxt.fillText("z",obj.posx+70,obj.posy);
						}
						if(allT%gp>=gp*2/4){
							cxt.fillText("z",obj.posx+75,obj.posy-9);
						}
						if(allT%gp>=gp*3/4){
							
							cxt.fillText("z",obj.posx+80,obj.posy-18);
						}
		...
		//开始发动手牌法术的效果
					...
					function domagic(whoattack,idx,goal,goalidx){
						if(whoattack[idx].name=='火球术'){
							...
							tx.push({funname:"killTX",fun:killTX,posx:goal[goalidx].posx,posy:goal[goalidx].posy,beginT:allT,lastT:8});
							goal[goalidx].posi = 3;
							whoattack[idx].pos =3;
						...
						for(var i=0;i<mycard.length;i++){
							if(mycard[i].posi==1){				//看是否选中手中的卡
								//到moveArray数组里找这张卡 看这张卡是否正在移动，如果在移动，那么就不能选中,
									var canchoose = 1;
									for(var kk=0;kk<moveArray.length;kk++){
